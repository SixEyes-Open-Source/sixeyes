...
void MotorControlScheduler::start() {
    ...
    // Runs on core 0 (default), high priority to prevent preemption by low-priority tasks
    ...
    if (result == pdPASS) {
        running_ = true;
        Serial.println("MotorControlScheduler: control loop task created successfully");
    } else {
...
bool MotorControlScheduler::running() const {
    return running_;
}

uint32_t MotorControlScheduler::getLoopCount() const {
    return loop_count_;
}

float MotorControlScheduler::getAverageLoopTimeMs() const {
    if (loop_count_ == 0) return 0.0f;

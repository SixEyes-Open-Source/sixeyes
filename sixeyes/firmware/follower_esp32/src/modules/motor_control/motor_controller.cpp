...
inline float signf_nonzero(float v) {
...
void MotorController::stepTimerCallback(void* arg) {
    ...
    for (size_t motor_index = 0; motor_index < NUM_STEPPERS; ++motor_index) {
        ...
        if (emit > 0) {
            ...
            self->step_position_isr_[motor_index] += emit_direction_positive[motor_index] ? emit : -emit;
        }
    ...
    for (size_t motor_index = 0; motor_index < NUM_STEPPERS; ++motor_index) {
        ...
        digitalWrite(TMC2209_DIR_PINS[motor_index], emit_direction_positive[motor_index] ? HIGH : LOW);
...
void MotorController::setCommandedStepRate(uint8_t motor_index, float steps_per_s) {
    const float max_step_rate = MAX_MOTOR_SPEED_DEG_PER_S / kDegreesPerStep;
    const float clamped = std::max(-max_step_rate, std::min(max_step_rate, steps_per_s));
...
void MotorController::init() {
    ...
    Serial.println("MotorController: init timer-driven planner + step generator (open-loop)");
...
void MotorController::enforceShoulderAntiPhaseLock() {
    ...
    const int32_t shoulder_a_relative_steps = target_steps_[SHOULDER_A] - zero_offsets_steps_[SHOULDER_A];
    target_steps_[SHOULDER_B] = zero_offsets_steps_[SHOULDER_B] - shoulder_a_relative_steps;
    target_positions_[SHOULDER_B] = stepsToDegrees(target_steps_[SHOULDER_B] - zero_offsets_steps_[SHOULDER_B]);
}
...
void MotorController::setAbsoluteTargets(const std::array<float, NUM_STEPPERS> &targets) {
    ...
    for (int i = 0; i < NUM_STEPPERS; ++i) {
        ...
        target_steps_[i] = zero_offsets_steps_[i] + degreesToSteps(target_positions_[i]);
    }
...
void MotorController::updatePlannerTargets(float dt) {
    ...
    for (size_t i = 0; i < NUM_STEPPERS; ++i) {
        ...
        max_remaining_steps = std::max(max_remaining_steps, std::abs(remaining_steps[i]));
    }
    ...
    for (size_t i = 0; i < NUM_STEPPERS; ++i) {
        ...
        if (abs_rem > 1 && max_remaining_steps > 0) {
            const float sync_scale = static_cast<float>(abs_rem) / static_cast<float>(max_remaining_steps);
            const float sync_target = max_step_rate * sync_scale;
            const float brake_limit = sqrtf(2.0f * max_step_accel * static_cast<float>(abs_rem));
            const float limited = std::min(sync_target, brake_limit);
            desired_vel = signf_nonzero(static_cast<float>(rem)) * limited;
        }

        float accel_target = (desired_vel - planner_velocity_steps_s_[i]) / std::max(0.0005f, dt);
        accel_target = std::max(-max_step_accel, std::min(max_step_accel, accel_target));

        const float max_d_accel = max_step_jerk * dt;
        const float accel_error = accel_target - planner_accel_steps_s2_[i];
        planner_accel_steps_s2_[i] += std::max(-max_d_accel, std::min(max_d_accel, accel_error));

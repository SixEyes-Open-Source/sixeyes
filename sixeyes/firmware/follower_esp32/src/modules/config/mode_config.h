...
// ============================================================================
  ...
  #error "OPERATION_MODE not recognized. Set via: -DOPERATION_MODE=MODE_VLA_INFERENCE or MODE_TELEOPERATION"
...
// Control loop timing
// Use board_config.h for canonical CONTROL_LOOP_HZ to avoid duplicate definitions
...
// Motor safety
#define MOTOR_DISABLE_LATENCY_MS        (CONTROL_LOOP_PERIOD_MS)  // Must disable in 1 cycle
...
#define MODE_NAME() \
  (OPERATION_MODE == MODE_VLA_INFERENCE ? "VLA_INFERENCE" : \
   OPERATION_MODE == MODE_TELEOPERATION ? "TELEOPERATION" : "UNKNOWN")

...
HeartbeatReceiver::HeartbeatReceiver() : rx_index_(0) {
    clearRxBuffer();
}
...
bool HeartbeatReceiver::parseHeartbeatPacket(const char *line, uint32_t &source_id, uint32_t &seq) {
    ...
    int result = sscanf(line, "HB:%lu,%lu", (unsigned long *)&source_id, (unsigned long *)&seq);
...
bool ROS2SafetyBridge::isROS2Connected() const {
    return ros2_connected_;
}

uint32_t ROS2SafetyBridge::getLastSequence() const {
    return last_seq_;
}

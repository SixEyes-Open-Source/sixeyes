...
size_t UsbCDC::buildFrame(const uint8_t *payload, size_t payload_len, uint8_t *frame) {
...
bool UsbCDC::sendTelemetry(const char *json_payload) {
    ...
    size_t frame_len = buildFrame((const uint8_t *)json_payload, payload_len, frame_buffer_);
...
bool UsbCDC::isConnected() const {
    return connected_;
}

uint32_t UsbCDC::getPacketsSent() const {
    return packets_sent_;
}

uint32_t UsbCDC::getPacketsDropped() const {
...
size_t TelemetryCollector::formatJSON(const TelemetryPacket &packet, char *buf, size_t buf_len) {
    int written = snprintf(
        ...
        "\"servo\":[%.1f,%.1f,%.1f]}",
        packet.timestamp_ms,
        packet.motors_enabled,
        packet.safety_fault,
        packet.stepper_positions_deg[0],
        packet.stepper_positions_deg[1],
        packet.stepper_positions_deg[2],
        packet.stepper_positions_deg[3],
        packet.stepper_velocities_deg_sec[0],
        packet.stepper_velocities_deg_sec[1],
        packet.stepper_velocities_deg_sec[2],
        packet.stepper_velocities_deg_sec[3],
        packet.servo_positions_deg[0],
        packet.servo_positions_deg[1],
        packet.servo_positions_deg[2]
    );

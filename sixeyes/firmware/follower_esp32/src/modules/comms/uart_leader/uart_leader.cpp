...
bool UartLeader::readAbsoluteTargets(std::array<float, NUM_STEPPERS> &out_targets, uint32_t &out_seq) {
    // Non-blocking parser: accumulate bytes and process when a full packet is available.
    ...
    while (idx < rx_len_) {
        switch (state_) {
            case ParseState::FIND_SYNC: {
                if (idx + 1 >= rx_len_) { idx = rx_len_; break; }
            ...
            case ParseState::READ_HEADER: {
                ...
                if (header_remaining < header_size) { idx = rx_len_; break; }
                msg_type_ = rx_buf_[idx];
                seq_ =  (uint32_t)rx_buf_[idx+1] | ((uint32_t)rx_buf_[idx+2] << 8) | ((uint32_t)rx_buf_[idx+3] << 16) | ((uint32_t)rx_buf_[idx+4] << 24);
                timestamp_ = 0;
                for (int b = 0; b < 8; ++b) { timestamp_ |= (uint64_t)rx_buf_[idx+5+b] << (8*b); }
                payload_len_ = (uint16_t)rx_buf_[idx+13] | ((uint16_t)rx_buf_[idx+14] << 8);
                expected_len_ = 2 + header_size + payload_len_ + 4;
                if (payload_len_ > 256) { idx = idx + 1; state_ = ParseState::FIND_SYNC; break; }
                idx += header_size;
            ...
            case ParseState::READ_PAYLOAD: {
                ...
                if (have < payload_len_) { idx = rx_len_; break; }
                idx += payload_len_;
            ...
            case ParseState::READ_CRC: {
                const size_t header_size = 1 + 4 + 8 + 2;
                if (rx_len_ - idx < 4) { idx = rx_len_; break; }
                uint32_t received_crc = (uint32_t)rx_buf_[idx] | ((uint32_t)rx_buf_[idx+1] << 8) | ((uint32_t)rx_buf_[idx+2] << 16) | ((uint32_t)rx_buf_[idx+3] << 24);
                size_t packet_end = idx + 4; // exclusive
                size_t msg_type_pos = packet_end - (header_size + payload_len_ + 4) + 2; // +2 to skip sync
                if (msg_type_pos + 1 > rx_len_) { rx_len_ = 0; state_ = ParseState::FIND_SYNC; idx = 0; break; }
                ...
                if (calc_crc != received_crc) {
                    if (rx_len_ > 0) { memmove(rx_buf_, rx_buf_ + 1, rx_len_ - 1); rx_len_ -= 1; }
                    state_ = ParseState::FIND_SYNC; idx = 0; break;
                }
                ...
                if (msg_type_ == 0x01) {
                    if (payload_len_ == NUM_STEPPERS * sizeof(float)) {
                        ...
                        if (remain > 0) memmove(rx_buf_, rx_buf_ + consumed, remain);
                        rx_len_ = remain;
                        state_ = ParseState::FIND_SYNC;
                        return true;
                    } else { rx_len_ = 0; state_ = ParseState::FIND_SYNC; idx = 0; break; }
